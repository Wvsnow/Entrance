...
public class Painter extends JFrame implements ActionListener {
    ...
    private static final long serialVersionUID = 657900794110605040L;
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == jMenuItem[1][0]) {
            ...
            FileDialog fileDialog = new FileDialog(new Frame(), "閬告搰涓�鍊嬪湒妾�",
                    FileDialog.LOAD);
            fileDialog.show();
            if (fileDialog.getFile() == null)
                return;
            ...
            if (drawPanel.filename == null) {
                ...
                try {
                    ...
                    JOptionPane.showMessageDialog(null, even.toString(),
                            "鐒℃硶鍎插瓨鍦栨獢", JOptionPane.ERROR_MESSAGE);
                }
            }
            ...
            JOptionPane
                    .showMessageDialog(
                            null,
                            "绋嬪簭鍚嶇О锛氬皬鐢诲 (2004/6/2)\n浣滆�咃細鏈变紵绾禱n绯荤礆锛氭贰姹熷ぇ瀛﹁祫璁伐绋�1-C\n淇＄锛歸eiluen@pie.com.tw\n\n鐗堟湰鐗圭偣锛氬棥\n",
                            "浜� 灏忕敾瀹�", 1, new ImageIcon("img/paint.gif"));
        }
        for (i = 0; i < 2; i++) {
    ...
    public class SetPanel extends JPanel implements ItemListener,
        ...
        private JPanel temp0 = new JPanel(new GridLayout(4, 1)),
                ...
                temp6 = new JPanel(new FlowLayout(FlowLayout.LEFT)),
                temp7 = new JPanel(new FlowLayout(FlowLayout.LEFT)),
                temp8 = new JPanel(new GridLayout(3, 1));

        public JCheckBox jCheckBox = new JCheckBox();
        private BufferedImage bufImg = new BufferedImage(50, 50,
        ...
        public SetPanel() {// 产生版面//
            ...
            jPanel_set1.setBorder(new TitledBorder(null, "边框",
            ...
            jPanel_set2.setBorder(new TitledBorder(null, "扇型设定",
                    TitledBorder.LEFT, TitledBorder.TOP));
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (e.getSource() == style[0])
                ...
                stroke = new BasicStroke(stroke2.getLineWidth(),
                        BasicStroke.CAP_BUTT, stroke2.getLineJoin(),
                        stroke2.getMiterLimit(), stroke2.getDashArray(),
                        stroke2.getDashPhase());
        ...
        public void itemStateChanged(ItemEvent e) {
            ...
            if (e.getSource() == jCheckBox) {
                if (e.getStateChange() == ItemEvent.SELECTED)
                    stroke = new BasicStroke(stroke2.getLineWidth(),
                            stroke2.getEndCap(), stroke2.getLineJoin(), 10,
                            data, 0);
                else
                    stroke = new BasicStroke(stroke2.getLineWidth(),
                            stroke2.getEndCap(), stroke2.getLineJoin());
            }
        }
    ...
    public class Gradient extends JPanel {// 预览用
    ...
    public class ColorPanel extends JPanel implements MouseListener,
        ...
        private BufferedImage bufImg = new BufferedImage(12, 12,
                BufferedImage.TYPE_3BYTE_BGR), bufImg2 = new BufferedImage(12,
        ...
        private int rgb[][] = {
                {0, 255, 128, 192, 128, 255, 128, 255, 0, 0, 0, 0, 0, 0, 128,
                        255, 128, 255, 0, 0, 0, 128, 0, 128, 128, 255, 128,
                        255, 255, 255, 255, 255},
                {0, 255, 128, 192, 0, 0, 128, 255, 128, 255, 128, 255, 0, 0,
                        0, 0, 128, 255, 64, 255, 128, 255, 64, 128, 0, 0, 64,
                        128, 255, 255, 255, 255},
                {0, 255, 128, 192, 0, 0, 0, 0, 0, 0, 128, 255, 128, 255, 128,
                        255, 64, 128, 64, 128, 255, 255, 128, 255, 255, 128, 0,
                        64, 255, 255, 255, 255}};
        ...
        public ColorPanel() {// 产生版面//
            ...
            for (i = 0; i < jPanel_color0.length; i++) {
                ...
                if (i <= 2) {
                    jPanel_color0[i].setBorder(BorderFactory
                            .createEtchedBorder(BevelBorder.RAISED));
                    jPanel_color0[i].setLayout(null);
                } else {
                    jPanel_color0[i].setBackground(new Color(rgb[0][i - 3],
                            rgb[1][i - 3], rgb[2][i - 3]));
            ...
            for (i = 0; i < jPanel_color2.length; i++) {
                ...
                jPanel_color2[i].setBackground(new Color(rgb[0][i], rgb[1][i],
                        rgb[2][i]));
                if (i >= 28)
            ...
            for (i = 0; i < jPanel_color1.length; i++) {
                ...
                if (i % 2 == 0) {
                    jPanel_color1[i].setBounds(new Rectangle(32 + i / 2 * 16,
                            0, 16, 16));
                } else {
                    jPanel_color1[i].setBounds(new Rectangle(32 + i / 2 * 16,
                            16, 16, 16));
                }
                jPanel_color1[i].setBorder(BorderFactory
                        .createEtchedBorder(BevelBorder.RAISED));
            }
            ...
            jDialog = new JDialog(Painter.this, "请选择两种颜色做渐变", true);
        ...
        public void actionPerformed(ActionEvent e) {
            if (e.getSource() == left) {
                center.G_color_left = JColorChooser.showDialog(Painter.this,
                        "請選擇邊線顏色", center.G_color_left);
                center.repaint();
            } else if (e.getSource() == right) {
                center.G_color_right = JColorChooser.showDialog(Painter.this,
                        "請選擇邊線顏色", center.G_color_right);
                center.repaint();
            } else {
                jDialog.dispose();
            }
        ...
        public void mousePressed(MouseEvent e) {
            Graphics2D g2d;
            if (e.getX() >= 5 && e.getX() <= 20 && e.getY() >= 5
                    && e.getY() <= 20) {
                g2d = bufImg.createGraphics();
                color_border = JColorChooser.showDialog(Painter.this,
                        "請選擇邊線顏色", (Color) color_border);
                ...
                color_inside = JColorChooser.showDialog(Painter.this,
                        "請選擇填充顏色", (Color) color_inside);
                g2d.setPaint(color_inside);
                g2d.fill(new Rectangle2D.Double(0, 0, 12, 12));
                repaint();
            }

            if (!(e.getX() >= 32 && e.getX() <= 288))
                return;
            ...
            if (choose == 28) {// 填充無顏色
                ...
                g2d.setPaint(new GradientPaint(0, 0, center.G_color_left, 12,
                        12, center.G_color_right, true));
                g2d.fill(new Rectangle2D.Double(0, 0, 12, 12));
                repaint();

                if (e.getButton() == 1) {
                    color_border = new GradientPaint(0, 0, center.G_color_left,
                            12, 12, center.G_color_right, true);
                } else {
                    color_inside = new GradientPaint(0, 0, center.G_color_left,
                            12, 12, center.G_color_right, true);
                }
            } else if (choose == 30) {// 填充圖案
                FileDialog fileDialog = new FileDialog(new Frame(), "選擇一個圖檔",
                        FileDialog.LOAD);// 利用FileDialog抓取檔名
                ...
                icon = new ImageIcon(fileDialog.getDirectory()
                        + fileDialog.getFile());// 利用FileDialog傳進來的檔名讀取圖片
                BufferedImage bufferedImage = new BufferedImage(
                        icon.getIconWidth(), icon.getIconHeight(),
                        BufferedImage.TYPE_3BYTE_BGR);// 創一張新的BufferedImage，為了要讀取讀進來的圖片長寬，以免有空白
                bufferedImage.createGraphics().drawImage(icon.getImage(), 0, 0,
                        this);// 把icon畫到BufferedImage上
                repaint();// 重繪螢幕

                if (e.getButton() == 1)// 判斷邊線顏色或內部填滿色
                    color_border = new TexturePaint(bufferedImage,
                            new Rectangle(icon.getIconWidth(),
                                    icon.getIconHeight()));// 把這張BufferedImage設成TexturePaint來填滿
                else
                ...
                FontColor = JColorChooser.showDialog(Painter.this,
                        "請選擇一個顏色當文字顏色", FontColor);// 讓使用者選擇顏色

                g2d.drawImage(special_color[3].getImage(), 0, 0, this);// 把調色盤左方換成『字』

                BufferedImage bufferedImage = new BufferedImage(
                        draw_panel_width, draw_panel_height,
                        BufferedImage.TYPE_3BYTE_BGR);// 創一張新的BufferedImage
                Graphics2D g2d_bufferedImage = bufferedImage.createGraphics();// 把Graphics拿出來畫

                FontRenderContext frc = g2d_bufferedImage
                        .getFontRenderContext();// 讀Graphics中的Font
                ...
                int sw = (int) (tl.getBounds().getWidth() + tl
                        .getCharacterCount());// 計算TextLayout的長
                int sh = (int) (tl.getBounds().getHeight() + 3);// 計算TextLayout的高

                bufferedImage = new BufferedImage(sw, sh,
                        BufferedImage.TYPE_3BYTE_BGR);// 再創一張新的BufferedImage，這裡利用相同指標指向不同記憶體
                ...
                if (e.getButton() == 1)// 判斷邊線顏色或內部填滿色
                    color_border = new TexturePaint(bufferedImage,
                            new Rectangle(sw, sh));// 把這張BufferedImage設成TexturePaint來填滿
                else
                    color_inside = new TexturePaint(bufferedImage,
                            new Rectangle(sw, sh));
            } else {// 填充一般色
                ...
                if (e.getButton() == 1)
                    color_border = new Color(rgb[0][choose], rgb[1][choose],
                            rgb[2][choose]);
                else
                    color_inside = new Color(rgb[0][choose], rgb[1][choose],
                            rgb[2][choose]);
            }
        }
    ...
    public class DrawPanel extends JPanel implements MouseListener,
        ...
        // public BufferedImage bufImg;// 瑷橀寗鏈�鏂扮暙闈紝涓﹀湪姝や笂浣滅暙
        // private BufferedImage bufImg_data[];//
        // 瑷橀寗鎵�鏈夌暙鍑哄湒闈紝绱㈠紩鍊艰秺澶ц秺鏂帮紝鏈�澶х偤鏈�鏂�
        ...
        private ImageIcon img;
        private JLabel jlbImg;
        private int x1 = -1, y1 = -1, x2, y2, count, redo_lim, press, temp_x1,
                temp_y1, temp_x2, temp_y2, temp_x3, temp_y3, step, step_chk,
                step_arc, step_chk_arc, chk, first, click, cut;
        private Arc2D.Double arc2D = new Arc2D.Double();// 鎵囧瀷
        private Line2D.Double line2D = new Line2D.Double();// 鐩寸窔
        private Ellipse2D.Double ellipse2D = new Ellipse2D.Double();// 姗㈠湏
        private Rectangle2D.Double rectangle2D = new Rectangle2D.Double();// 鐭╁瀷
        private CubicCurve2D.Double cubicCurve2D = new CubicCurve2D.Double();// 璨濇皬鏇茬窔
        private RoundRectangle2D.Double roundRectangle2D = new RoundRectangle2D.Double();// 鍦撹鐭╁瀷
        private Polygon polygon;// 澶氶倞鍨�
        private float data[] = {5};
        private Rectangle2D.Double rectangle2D_select = new Rectangle2D.Double();// 鐭╁瀷
        ...
        public void resize() {// 鏀硅畩澶у皬
            bufImg = new BufferedImage(draw_panel_width, draw_panel_height,
                    BufferedImage.TYPE_3BYTE_BGR);
            jlbImg = new JLabel(new ImageIcon(bufImg));// 鍦↗Label涓婃斁缃産ufImg锛岀敤渚嗙躬鍦�
            this.removeAll();
            this.add(jlbImg);
            ...
            // 瑷橀寗鍙噸鍋氭渶澶ф鏁革紝涓﹁畵閲嶅仛涓嶅彲鎸�//
            redo_lim = count++;
            jMenuItem[1][1].setEnabled(false);

            // 鏂板涓�寮礏ufferedImage鍨嬫厠鑷砨ufImg_data[count]锛屼甫灏嘼ufImg绻＝鑷砨ufImg_data[count]//
        ...
        public DrawPanel() {
            ...
            // Font//
            jDialog = new JDialog(Painter.this, "璜嬮伕鎿囨枃瀛椼�佸瓧鍨嬨�佸ぇ灏忚垏灞��", true);
            fontsize.setValue(new Integer(100));
            bold = new JCheckBox("绮楅珨", true);
            italic = new JCheckBox("鏂滈珨", true);
            ok = new JButton("纰哄畾");
            ...
            temp_5.add(new JLabel("鏂囧瓧:"));
            temp_5.add(textField_word);
            temp_1.add(new JLabel("瀛楅珨:"));
            temp_1.add(textField_font);
            temp_2.add(new JLabel("澶у皬:"));
            temp_2.add(fontsize);
            temp_3.add(new JLabel("灞��:"));
            temp_3.add(bold);
            temp_3.add(italic);
            temp_4.add(ok);
            temp_4.add(cancel);
        ...
        public void openfile(String filename) {// 闁嬪暉鑸婃獢
            Graphics2D g2d_bufImg = (Graphics2D) bufImg.getGraphics();
        ...
        public void cut() {
            ...
            BufferedImage copy = bufImg.getSubimage(
                    (int) rectangle2D_select.getX(),
                    (int) rectangle2D_select.getY(),
                    (int) rectangle2D_select.getWidth(),
                    (int) rectangle2D_select.getHeight());
            Graphics2D g2d_bufImg_cut = (Graphics2D) bufImg_cut
                    .createGraphics();
            ...
            g2d_bufImg.fill(new Rectangle2D.Double((int) rectangle2D_select
                    .getX(), (int) rectangle2D_select.getY(),
                    (int) rectangle2D_select.getWidth(),
                    (int) rectangle2D_select.getHeight()));
        ...
        public void mouseReleased(MouseEvent e) {
            ...
            if (step_chk == 0)// 鎺у埗璨濇皬鏇茬窔鐢�
                step = 1;
            else if (step_chk == 1)
                step = 2;

            if (step_chk_arc == 0)// 鎺у埗鎵囧瀷鐢�
                chk = step_arc = 1;
            ...
            if ((step_chk == 2 && step == 2)
                    || (step_chk_arc == 2 && step_arc == 2) || drawMethod == 0
                    || drawMethod == 1 || drawMethod == 2 || drawMethod == 3
                    || drawMethod == 7 || drawMethod == 8 || drawMethod == 9
                    || cut == 2) {// 鐣朵笉鏄暙璨濇皬鏇茬窔鎴栨槸宸茬稉瀹屾垚璨濇皬鏇茬窔鏅傜暙
                toDraw();
            }
        }
        ...
        public void draw(int input_x1, int input_y1, int input_x2, int input_y2) {
            if (drawMethod == 0) {// 鐩寸窔鏅傦紝璁搒hape鐐篖ine2D
                shape = line2D;
                line2D.setLine(input_x1, input_y1, input_x2, input_y2);
            } else if (drawMethod == 1) {// 鐭╁瀷鏅傦紝璁搒hape鐐篟ectangle2D
                shape = rectangle2D;
                rectangle2D.setRect(Math.min(input_x1, input_x2),
                        Math.min(input_y1, input_y2),
                        Math.abs(input_x1 - input_x2),
                        Math.abs(input_y1 - input_y2));
            } else if (drawMethod == 2) {// 姗㈠湏鏅�
                shape = ellipse2D;
                ellipse2D.setFrame(Math.min(input_x1, input_x2),
                        Math.min(input_y1, input_y2),
                        Math.abs(input_x1 - input_x2),
                        Math.abs(input_y1 - input_y2));
            } else if (drawMethod == 3) {// 鍦撹鐭╁瀷
                shape = roundRectangle2D;
                roundRectangle2D.setRoundRect(Math.min(input_x1, input_x2),
                        Math.min(input_y1, input_y2),
                        Math.abs(input_x1 - input_x2),
                        Math.abs(input_y1 - input_y2), 10.0f, 10.0f);
            } else if (drawMethod == 4) {// 璨濇皬鏇茬窔
                shape = cubicCurve2D;
                if (step == 0) {
                    cubicCurve2D.setCurve(input_x1, input_y1, input_x1,
                            input_y1, input_x2, input_y2, input_x2, input_y2);
                    temp_x1 = input_x1;
                    temp_y1 = input_y1;
                    temp_x2 = input_x2;
                    temp_y2 = input_y2;
            ...
            } else if (drawMethod == 5) {// 鎵囧瀷锛宑hk鐢ㄤ締闃叉鎰忓鐨剅epaint//
                if (step_arc == 0 || chk == 1) {// 姝ラ鎺у埗
                    shape = ellipse2D;
                    ellipse2D.setFrame(Math.min(input_x1, input_x2),
                            Math.min(input_y1, input_y2),
                            Math.abs(input_x1 - input_x2),
                    ...
                    temp_y2 = input_y2;
                    step_chk_arc = 0;
                } else if (step_arc == 1 || chk == 2) {// 姝ラ鎺у埗
                    shape = arc2D;
                    ...
                    arc2D.setArc(Math.min(temp_x1, temp_x2),
                            Math.min(temp_y1, temp_y2),
                            Math.abs(temp_x1 - temp_x2),
                            Math.abs(temp_y1 - temp_y2), start, 0, pie_shape);
                    step_chk_arc = 1;
                } else if (step_arc == 2 || chk == 3) {// 姝ラ鎺у埗
                    shape = arc2D;
                    ...
                    arc2D.setArc(Math.min(temp_x1, temp_x2),
                            Math.min(temp_y1, temp_y2),
                            Math.abs(temp_x1 - temp_x2),
                            Math.abs(temp_y1 - temp_y2), start, end, pie_shape);
                    step_chk_arc = 2;
                }
            } else if (drawMethod == 6) {// 澶氶倞鍨�
                shape = polygon;
            } else if (drawMethod == 7 || drawMethod == 8) {// 浠绘剰绶氾紗姗＄毊鎿�
                Graphics2D g2d_bufImg = (Graphics2D) bufImg.getGraphics();
            ...
            if (color_border instanceof GradientPaint) {// 浣跨敤婕稿堡濉壊璁�鍙栨嫋鎷夊骇妯�
                color_border = new GradientPaint(input_x1, input_y1,
                        (Color) ((GradientPaint) color_border).getColor1(),
                        input_x2, input_y2,
                        (Color) ((GradientPaint) color_border).getColor2(),
                        true);
            }
            if (color_inside instanceof GradientPaint) {
                color_inside = new GradientPaint(input_x1, input_y1,
                        (Color) ((GradientPaint) color_inside).getColor1(),
                        input_x2, input_y2,
                        (Color) ((GradientPaint) color_inside).getColor2(),
                        true);
            }
        }
        ...
        public void paint(Graphics g) {
            ...
            if (cut == 1) {
                g2d.setPaint(Color.black);
                g2d.setStroke(basicStroke_select);
                rectangle2D_select.setRect(select_x, select_y, select_w,
                        select_h);
                g2d.draw(rectangle2D_select);
            }
            if (cut == 2) {
            ...
            if (drawMethod == 7 || drawMethod == 8) {
                ...
                ellipse2D_pan
                        .setFrame(x2 - setPanel.number / 2, y2
                                        - setPanel.number / 2, setPanel.number,
                                setPanel.number);
